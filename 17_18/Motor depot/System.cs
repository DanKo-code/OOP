...
namespace Motor_depot
{
    internal class System
    {
        ...
        public uint MakeSentOrder(string userName, uint carCarring, uint carSpeed)
        {
            ...
            //Вставить логику для Abstract Factory!!!

            if (carCarring <= 20)
            {
                Databases.OrderList.Add(
            ...
            else if (carCarring <= 50)
            {
                Databases.OrderList.Add(
                    ...
                    (Domain.Order)((new MediumCarOrderFactory(orderID, carCarring, carSpeed)).GetOrder())


                    );
            }
            else if (carCarring <= 90)
            {
                Databases.OrderList.Add(


                    (Domain.Order)((new BigCarOrderFactory(orderID, carCarring, carSpeed)).GetOrder())
